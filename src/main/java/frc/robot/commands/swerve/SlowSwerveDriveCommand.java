...
public class SlowSwerveDriveCommand extends CommandBase {
  ...
  public double calculateDrivePercentage(double JoystickPercent, double TriggerPercent) {
    ...
    if (Math.abs(JoystickPercent) > 0.05) {
      ...
      OutputSpeedPercent = JoystickPercent + ((JoystickPercent / 0.2) * TriggerPercent);
    } else {
  ...
  public void execute() {
    ...
    m_drive.slowDrive(drive, strafe, rotation, m_isFieldRelative, true); // Forward/Back drive, Left/Right Strafe,
                                                                         // Left/Right Turn
