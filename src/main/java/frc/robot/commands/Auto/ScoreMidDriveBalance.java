...
public class ScoreMidDriveBalance extends SequentialCommandGroup {
  /** Creates a new ScoreMidDriveBack. */

  // defineing the requirements for motors it needs to run.
  public ScoreMidDriveBalance(DriveSubsystem m_drive, FieldSim m_fieldSim, ArmInOut m_ArmInOut, ArmPivot m_ArmPivot,
    ...
    addCommands(
        ...
        new RunCommand(() -> m_drive.drive(0.1, 0, 0, true, true), m_drive).withTimeout(6.5),
        new RunCommand(() -> m_drive.drive(0, 0, 0.05, true, true), m_drive).withTimeout(1.45),
        new RunCommand(() -> m_drive.drive(-0.1, 0, 0, true, true), m_drive).withTimeout(3),
        new InstantCommand(m_LED::autoEnd),
        new BalanceChargeStation(m_drive, true)
